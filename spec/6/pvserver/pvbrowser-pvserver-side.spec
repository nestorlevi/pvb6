...
%if 0%{?suse_version}
Requires:      libpng
BuildRequires: tcsh clang gcc gcc-c++ libreadline6 readline-devel fdupes cmake
%endif

%if 0%{?fedora_version}
# Requires:  ld-linux.so.2
BuildRequires: clang libstdc++-devel lcms-devel readline-devel fdupes
# BuildRequires: gcc-c++ libstdc++-devel lcms-devel readline-devel fdupes
%endif

%if 0%{?rhel_version}
# Requires:  ld-linux.so.2
BuildRequires: gcc-c++ libstdc++-devel readline-devel csh 
%endif

%if 0%{?centos_version}
# Requires:  ld-linux.so.2
BuildRequires: pkgconfig
BuildRequires: gcc-c++ libstdc++-devel readline-devel
%endif
...
%description
 ...
 Commands that start qt based applications:
   pvbrowser
   pvdevelop
...
#!/bin/bash
###########################################################
echo start-of-pvbrowser-pvserver-side.spec
rm -f ./pvsexample/*.o
rm -f ./pvsexample/pvsexample
# ./build.sh all buildservice
# ./build.sh all buildservice
./rlbuild-pvserver-side-only.sh all buildservice
./rlbuild-pvserver-side-only.sh all buildservice
...
%package     devel
Summary:     Package pvbrowser server side
Group:       Productivity/Graphics/Visualization/Other
License:     GPL
