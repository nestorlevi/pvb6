...
void QImageWidget::setImage(const QImage *newimage)
{
  ...
  else if(w > image.width() || h > image.height())
  {
    ...
    QImage rlmurx_ignore = image.scaled(w, h, Qt::KeepAspectRatio, Qt::SmoothTransformation);
  }
  clearMask();
  perhapsSetMask();
  w = image.width();
...
void QImageWidget::enterEvent(QEnterEvent *event)
{
  char buf[100];
...
{
  if(image.load(filename)) // rl06nov2023
  {			  
    if(rotate)
    {
      QTransform m;
